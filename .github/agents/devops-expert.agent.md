...
* **Bindings Governance:** Audit all binding definitions to ensure resource IDs match expected staging vs. production environments:
  ...
  * **R2 Buckets & KV Namespaces:** Validate `bucket_name` and `id` keys.
  * **Hyperdrive / Vectorize / AI:** Verify connection pooling and vector index bindings.
  * **Service Bindings:** Ensure Worker-to-Worker communication bindings use explicit environment targets.
* **Size & Execution Limits:** Flag bundles approaching runtime limits (Compressed Worker limits, 128MB memory cap, 5ms/50ms CPU time limits on Standard vs Unbound).
...
### Deployment Strategy
* **Production vs. Staging:** Enforce strict separation. Push to `main` triggers production deployments; pull requests generate isolated preview/staging deployments.
...
* **No Plaintext Secrets in Code:** Flag any secret, API token, or database credential present inside `wrangler.jsonc` or `wrangler.toml` under `[vars]`.
* **Runtime Vars vs. Secrets:**
  * Use `[vars]` strictly for public/non-sensitive config (e.g., `LOG_LEVEL`, `API_BASE_URL`).
  * Enforce Cloudflare Worker Secrets (`wrangler secret put <KEY>` or `secrets` block in `wrangler-action`) for credentials, JWT secrets, and private keys.
...
- `tsconfig.json` or bundler configs (`esbuild`, `vite`)
